  ...
  <nav className="bg-white/95 text-gray-700 p-3 sticky top-0 z-40 flex justify-between items-center border-b border-theme-primary/20 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          <button
            ...
            className="lg:hidden p-2 rounded-md hover:bg-theme-secondary/20 transition-colors"
          >
      ...
      <div id="mobile-drawer" aria-hidden={!open} className={`fixed inset-0 z-50 ${open ? 'block' : 'hidden'}`}>
        ...
        <aside
          ...
          role="dialog"
          aria-modal="true"
          className={`absolute left-0 top-0 h-full w-72 max-w-full bg-white border-r border-theme-primary/20 p-4 transform transition-transform duration-300 ease-in-out ${open ? 'translate-x-0' : '-translate-x-full'}`}
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <button aria-label="Cerrar menú" onClick={() => setOpen(false)} className="p-2 rounded-md hover:bg-theme-secondary/20 transition-colors">
            ...
            <ul className="space-y-1">
              {menuItems.map(item => (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className={`flex items-center px-3 py-2 rounded-md transition-colors ${pathname && pathname.startsWith(item.href) ? 'bg-theme-secondary/20 text-theme-primary' : 'text-gray-700 hover:bg-theme-secondary/20 hover:text-theme-primary'}`}
                  >
