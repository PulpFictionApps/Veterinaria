...
router.post('/', verifyToken, async (req, res) => {
  ...
  try {
    ...
    let supabasePdfUrl = null;
    let supabaseFileName = null;
    
    try {
      const pdfBuffer = fs.readFileSync(outPath);
      const supabaseFileName_temp = `receta-${Date.now()}-${pet.name.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`;
      
      const { data, error } = await supabase.storage
        .from('Prescriptions')
        .upload(supabaseFileName_temp, pdfBuffer, {
      ...
      if (error) {
        ...
        const { data: { publicUrl } } = supabase.storage
          .from('Prescriptions')
          .getPublicUrl(supabaseFileName_temp);
        
        supabasePdfUrl = publicUrl;
        supabaseFileName = supabaseFileName_temp;
        console.log('PDF uploaded to Supabase successfully:', publicUrl);
        ...
        try {
          ...
          console.log('Error cleaning up local file:', cleanupError);
        }
      }
    } catch (supabaseError) {
    ...
    const pdfUrl = supabasePdfUrl || `/tmp/${fileName}`;
    ...
    const prescription = await prisma.prescription.create({
      data: {
        ...
        pdfUrl,
        pdfPath: supabasePdfUrl ? null : outPath, // Solo guardar path local si no está en Supabase
        pdfFileName: supabaseFileName, // Nombre del archivo en Supabase
        sendWhatsApp,
        whatsappSent: false
      },
    });
...
router.get('/download/:id', verifyToken, async (req, res) => {
  try {
    ...
    if (prescription.pdfFileName) {
      try {
        console.log('Downloading from Supabase Storage:', prescription.pdfFileName);
        
        const { data, error } = await supabase.storage
          .from('Prescriptions')
          .download(prescription.pdfFileName);
        ...
        if (!error && data) {
          ...
          // Enviar el archivo desde Supabase
          const arrayBuffer = await data.arrayBuffer();
          res.send(Buffer.from(arrayBuffer));
          return;
        } else {
          console.log('Error downloading from Supabase or file not found:', error);
        }
      } catch (supabaseError) {
        console.log('Supabase download error:', supabaseError);
      }
    }
    ...
    if (!pdfPath || !fs.existsSync(pdfPath)) {
      ...
      return res.status(404).json({ 
        error: 'Archivo PDF no encontrado. El archivo puede haber sido eliminado.',
        details: 'No se pudo encontrar el archivo ni en Supabase Storage ni localmente'
      });
    }
    
    // Configurar headers para descarga local
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="${downloadName}"`);
